// ===========================================================================
//  link_plc.cpp — SLAVE transport over the KQ-330 power-line modem
//
//  Compiled only when LINK_WIFI is NOT defined.
//
//  The KQ-330 is a half-duplex UART bridge: bytes trickle across the mains at
//  roughly 100 bps regardless of the 9600 baud MCU-side link. Frames therefore
//  have to be reassembled byte by byte, and transmission needs a deliberate
//  inter-byte gap. Both of those concerns live here and nowhere else.
//
...
void link_begin() {
    ...
    Serial.printf("[LINK] PLC transport up (KQ-330, RX=GPIO%d TX=GPIO%d @ %d baud)\n",
                  PLC_RX_PIN, PLC_TX_PIN, PLC_BAUD);
}
...
uint8_t link_poll(uint8_t* buf, uint8_t maxlen) {
    ...
    if (rx_state != RX_WAIT_START &&
        ...
        Serial.printf("[LINK] PLC timeout - discarding %u partial byte(s): ", rx_buf_idx);
        for (uint8_t i = 0u; i < rx_buf_idx; i++) Serial.printf("0x%02X ", rx_buf[i]);
        Serial.println();
